...
from modeling.backbones.vit_pytorch import Trans
...
class TransSpecial(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        # 可以从blocks开始搞个分支出来：
        # 基本特征提取
        self.blocks_part1 = self.blocks[:10]  # 前半部分
        # 公共特征/特有特征
        self.blocks_part2 = self.blocks[10:]  # 后半部分
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        x = self.pos_drop(x)
        cash_x = []
        for index, blk in enumerate(self.blocks_part1):
            x = blk(x)
        part1 = x
        for index, blk in enumerate(self.blocks_part2):
            x = blk(x)
        cash_x.append(self.norm(x))
        return cash_x, part1

    def forward(self, x, cam_label=None, view_label=None):
        x, part1 = self.forward_features(x, cam_label, view_label)
        return x, part1  # 返回 part1 用作后续提取公共特征
...
def vit_base_patch16_224_mm(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0,
                         drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = TransSpecial(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True, \
