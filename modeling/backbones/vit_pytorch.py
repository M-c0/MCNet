...
class Attention(nn.Module):
    ...
    def forward(self, x, get_attn=False):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)  # 3,16,12,129,64
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)  16,12,129,64

        attn = (q @ k.transpose(-2, -1)) * self.scale  # 16,12,129,129  每个patch之间的相似度
        attn = attn.softmax(dim=-1)  # 16,12,129,129
        attn = self.attn_drop(attn)  # 16,12,129,129
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  # 16,129,768     16,12,129,64
        x = self.proj(x)
        x = self.proj_drop(x)
        if get_attn:
            return x, attn
...
class Trans(nn.Module):
    ...
    def forward(self, x, cam_label=None, view_label=None):
        x, att_list = self.forward_features(x, cam_label, view_label)
        return x, att_list
