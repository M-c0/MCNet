...
from layers.contrtastive_loss import MultiModalContrastiveLoss
from utils.meter import AverageMeter
from utils.metrics import R1_mAP_eval, R1_mAP
from torch.cuda import amp
import torch.distributed as dist

from visualize.visualize_emb import visualize_embeddings, visualize_embeddings_3d
...
def do_train(cfg,
             ...
             scheduler,
             loss_fn,
             num_query, local_rank, output_dir):
    ...
    loss_contrast = MultiModalContrastiveLoss()
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view, _) in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                ...
                loss = 0
                for i in range(0, 8, 2):  # 前8 个是三个模态和最终表征对应的 socre 和vector
                    loss_tmp = loss_fn(score=output[i], feat=output[i + 1], target=target, target_cam=target_cam)
                    loss = loss + loss_tmp

                # 根据当前模型的损失进行修改：
                loss_common = loss_fn(output[8], output[9], target, target_cam)  # 如果是common
                loss_c_glb = loss_contrast(output[10], target, target_cam)  # 如果是contrastive，
                loss_c_loc = 0
                for common_local in output[11]:  # 计算交集中的patch token 之间的损失
                    loss_c_loc = loss_c_loc + nn.MSELoss()(common_local[0], common_local[1]) + nn.MSELoss()(common_local[0], common_local[2]) + nn.MSELoss()(common_local[1], common_local[2])  # local token 的对比损失

                loss = loss + loss_common + loss_c_glb + loss_c_loc
            ...
            if (n_iter + 1) % log_period == 0:
                ...
                logger.info("Epoch[{}] Iteration[{}/{}] Loss: {:.3f}, Acc: {:.3f}, Base Lr: {:.2e}"
                            .format(epoch, (n_iter + 1), len(train_loader),
                                    loss_meter.avg, acc_meter.avg, scheduler._get_lr(epoch)[0])) # scheduler.get_lr()[0]))  # scheduler._get_lr(epoch)[0]
        ...
        if epoch % eval_period == 0 or epoch == 1 or (epoch + 1)>100:
            if cfg.MODEL.DIST_TRAIN:
                if dist.get_rank() == 0:
                    model.eval()
                ...
                if mAP >= best_index['mAP']:
                    ...
                    torch.save(model.state_dict(),
                               os.path.join(output_dir, cfg.MODEL.NAME + 'best.pth'))
...
def do_inference(cfg,
    ...
    for n_iter, (img, pid, camid, camids, target_view, imgpath) in enumerate(val_loader):
        with torch.no_grad():
            ...
            # 可视化 t-SNE
            # visualize_embeddings(feat, pid, save_path='./gradcam_vis/MSVR310/emb_v/baseline_2', iter=n_iter)  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            # visualize_embeddings_3d(feat, pid, save_path='./gradcam_vis/MSVR310/emb_v/all_3d_2', iter=n_iter)
