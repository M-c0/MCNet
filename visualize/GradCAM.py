...
def reshape_transform(tensor, height=8, width=16):
    """
...
def show_cam(index, imgpath, grayscale_cam, modality, cfg, n_iter):
    ...
    if cfg.DATASETS.NAMES == 'RGBNT201':
        ...
        img_path = f'../../datasets/{img_path}'
    elif cfg.DATASETS.NAMES == 'MSVR310':
        if modality == "RGB":
            img_path = f'../../datasets/{img_path}'
        elif modality == "NI":
            img_path = f'../../datasets/{img_path.replace("vis", "ni")}'
        elif modality == "TI":
            img_path = f'../../datasets/{img_path.replace("vis", "th")}'
    ...
    if cfg.DATASETS.NAMES == 'RGBNT100':
        ...
        rgb_image = cv2.resize(img, (256, 128))
        rgb_image = np.float32(rgb_image) / 255

    # Generate and save visualization
    visualization = show_cam_on_image(rgb_image, grayscale_cam)
    output_dir = f'../gradcam_vis/{cfg.DATASETS.NAMES}/{modality}/Baseline_73'  # +model name   baseline/common/PAT   PAT+common_l_g+ME
...
'''
    现在的可视化实际上只可视化了第三个模态的热力图，并没有可视化所有模态的热力图
'''
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", default="ReID_visualize.yml", help="Path to config file", type=str)
    ...
    # 设置输出目录和日志
    output_dir = cfg.OUTPUT_DIR+"/visualize"
    os.makedirs(output_dir, exist_ok=True)
    ...
    # 定义目标层
    target_layers = [model.backbone.base.blocks[-1].norm1]  # 不能直接定义base

    # 遍历验证集数据
    for n_iter, (img, pid, camids, camids_batch, viewids, imgpath) in enumerate(train_loader_normal):
        img = Newdict({'RGB': img['RGB'].to(device),
                       'NI': img['NI'].to(device),
                       'TI': img['TI'].to(device),
                       'cam_label': torch.tensor(camids).to(device)})
