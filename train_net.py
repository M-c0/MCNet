...
from solver.scheduler_factory import create_scheduler, WarmupMultiStepLR
...
if __name__ == '__main__':
    ...
    output_dir = cfg.OUTPUT_DIR + "/" + (str(datetime.now().year) + "-" + str(datetime.now().month)
                                      + "-" + str(datetime.now().day) + "-" + str(datetime.now().hour) + "_" + str(
                datetime.now().minute))
    ...
    do_train(
        ...
        scheduler,
        loss_func,
        num_query, args.local_rank, output_dir
    )
